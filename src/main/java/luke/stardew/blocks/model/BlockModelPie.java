...
public class BlockModelPie<T extends BlockLogic> extends BlockModelStandard<T> {
	...
	public BlockModelPie(Block<T> block) {
		super(block);
		float f = 0.0625F;
		float f1 = 0.375F;
	...
	public void renderSliceSide(Tessellator tessellator, Block<?> block, int x, int y, int z, Side side, boolean overrideTex) {
		...
		this.renderSide(tessellator, block.getBounds(), x, y, z, side, 0);
		if (overrideTex) {
	...
	public void renderSlice(Tessellator tessellator, Block<?> block, int x, int y, int z, int sliceX, int sliceZ) {
		...
		this.maxSlices = ((BlockLogicEdible)block.getLogic()).maxBites;
		//FIXME
		//block.setBlockBounds(xMin + offsetXMin, 0.0, zMin + offsetZMin, xMax - offsetXMax, 0.375, zMax - offsetZMax);
		this.renderSide(tessellator, aabb, x, y, z, Side.TOP, 0);
		this.renderSide(tessellator, aabb, x, y, z, Side.BOTTOM, 0);
