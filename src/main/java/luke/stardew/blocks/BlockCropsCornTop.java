...
@Deprecated
public class BlockCropsCornTop extends BlockLogicFlower implements IBonemealable {
	public BlockCropsCornTop(Block<?> block) {
		super(block);
	}
/*
	public BlockCropsCornTop(Block<?> block) {
	...
	public float getGrowthRate(World world, int x, int y, int z) {
		...
		for(int dx = x - 1; dx <= x + 1; ++dx) {
			for(int dz = z - 1; dz <= z + 1; ++dz) {
				...
				if (id == StardewBlocks.cropsCornBottom.id) {
					growthRateMod = 1.0F;
	...
	public ItemStack[] getBreakResult(World world, EnumDropCause dropCause, int x, int y, int z, int meta, TileEntity tileEntity) {
		return meta != 3 ? new ItemStack[]{new ItemStack(StardewItems.seedsCorn)} : new ItemStack[]{new ItemStack(StardewItems.seedsCorn, world.rand.nextInt(1) + 2), new ItemStack(StardewItems.corn, world.rand.nextInt(1))};
	}
 */
}
