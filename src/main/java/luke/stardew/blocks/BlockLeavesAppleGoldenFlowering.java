...
public class BlockLeavesAppleGoldenFlowering extends BlockLeavesAppleGolden {
	...
	public BlockLeavesAppleGoldenFlowering(Block<?> block) {
		super(block);
	}
	...
	public ItemStack[] getBreakResult(World world, EnumDropCause dropCause, int x, int y, int z, int meta, TileEntity tileEntity) {
		...
		if (dropCause != EnumDropCause.PICK_BLOCK && dropCause != EnumDropCause.SILK_TOUCH) {
			return growthRate == 0 ? null : new ItemStack[]{new ItemStack(Items.FOOD_APPLE_GOLD, world.rand.nextInt(1) + 1)};
		} else {
	...
	public boolean onBlockRightClicked(World world, int x, int y, int z, Player player, Side side, double xPlaced, double yPlaced) {
		...
		if (growthRate > 0) {
			...
			world.scheduleBlockUpdate(x, y, z, leavesAppleGoldenFlowering.id(), this.tickDelay());
			return true;
		} else {
	...
	public void updateTick(World world, int x, int y, int z, Random rand) {
		...
		if (world.getSeasonManager().getCurrentSeason() == Seasons.OVERWORLD_WINTER) {
			if (rand.nextInt(80) == 0 && growthRate == 0) {
				...
				world.scheduleBlockUpdate(x, y, z, StardewBlocks.leavesAppleGoldenFlowering.id(), this.tickDelay());
			}
